...
import { NON_SIGNAL_SYMBOL } from '../constants.js'
import type { NonSignal } from '../types/index.js'
...
export function markNonSignal<T extends AnyObject>(obj: T): NonSignal<T> {
  if (typeof obj !== 'object' || obj === null) {
    throw new TypeError('The argument must be an object type')
  }
  // 如果对象已经标记为非信号，则直接返回
  if ((obj as any)[NON_SIGNAL_SYMBOL]) {
    return obj as NonSignal<T>
  }
  Object.defineProperty(obj, NON_SIGNAL_SYMBOL, {
    value: true
  })
  return obj as NonSignal<T>
}
 ...
 * @returns { boolean } - 如果对象具有非信号标识符（NON_SIGNAL_SYMBOL），则返回true，否则返回false
 ...
 * ```
 */
export function isMarkNonSignal(obj: AnyObject): boolean {
  return !!(obj && (obj as any)?.[NON_SIGNAL_SYMBOL])
}

export { isMarkNonSignal as isMarkRaw }
