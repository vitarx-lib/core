...
/**
 * WeakSetReactive 是一个代理类，用于包装 WeakSet 或 Set 实例，提供额外的功能如信号触发。
 ...
 * const proxySet = new WeakSetReactive(originalSet).proxy;
 * const obj = {};
 ...
 */
export class WeakSetReactive<T extends AnySet | AnyWeakSet> extends ReactiveCollection<T> {
...
/**
 * SetReactive 是一个用于代理 Set 对象的类，继承自 WeakSetReactive。
 * 它主要用于自定义 Set 的某些行为，特别是 clear 操作。
 *
 * 核心功能：
 * - 自定义 Set 的 clear 行为
 * - 继承 WeakSetReactive 的其他所有代理功能
 *
 * 使用示例：
 * ```typescript
 * const mySet = new Set([1, 2, 3]);
 * const proxy = new SetReactive(mySet).proxy;
 ...
 * - 其他操作通过父类 WeakSetReactive 处理
 */
export class SetReactive<T extends AnySet> extends WeakSetReactive<T> {
  protected override doGet(target: T, p: string | symbol, receiver: any): any {
