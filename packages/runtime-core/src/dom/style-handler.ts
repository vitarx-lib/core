...
import type { ClassProperties, StyleProperties, StyleRules } from '../vnode/index.js'

/**
 * StyleHandler 类是一个用于处理 CSS 类和样式对象的工具类。
 ...
 * ```typescript
 * // 合并类名
 * const mergedClasses = StyleHandler.mergeCssClass('class1 class2', { class2: true, class3: true });
 * // 合并样式
 * const mergedStyles = StyleHandler.mergeCssStyle('color: red;', { fontSize: '14px' });
 * // 转换类名为数组
 * const classArray = StyleHandler.cssClassValueToArray('class1 class2');
 * // 转换样式对象为字符串
 * const styleString = StyleHandler.cssStyleValueToString({ color: 'red', fontSize: '14px' });
 * ```
 *
 * 注意事项：
