...
import { StatefulWidgetRuntime } from './Stateful.js'
 ...
 *
 * @param node - 无状态组件节点
 * @returns {StatelessWidgetRuntime} 管理器实例
 */
 ...
 *
 * @param node - 有状态组件节点
 * @returns {StatefulWidgetRuntime} 管理器实例
 */
 ...
 *
 * @param node - 组件节点
 * @returns {WidgetRuntime} 管理器实例
 */
export function createWidgetRuntime(node: WidgetNode): WidgetRuntime {
  if (!node.instance) {
    if (isStatelessWidget(node.type)) {
      new StatelessWidgetRuntime(node as StatelessWidgetNode)
    } else {
      new StatefulWidgetRuntime(node as StatefulWidgetNode)
    }
  }
