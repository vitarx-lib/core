...
import { resolveChildren, resolveProps } from '../compiler/utils.js'
...
> extends BaseView<ViewKind.ELEMENT> {
  ...
  constructor(tag: T, props: IntrinsicElements[T] | null, key?: unknown, location?: CodeLocation) {
    super(key, location)
    this.tag = tag
    ...
    if (ref) this.ref = ref
    this.activeScope = getActiveScope()
  }
  ...
  private setProps(node: HostElement, props: AnyProps, renderer: ViewRenderer): void {
    ...
    if (effects.length && this.activeScope?.state === 'active') {
      this.activeScope.onDispose(() => {
