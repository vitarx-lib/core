...
import { getFirstDomNode } from '../../src/client/utils.js'
...
describe('getFirstDomNode', () => {
  it('应该在容器为空时返回null', () => {
    ...
    const result = getFirstDomNode(container, 0)
  ...
  it('应该获取指定索引的节点', () => {
    ...
    const result = getFirstDomNode(container, 1)
  ...
  it('应该处理混合节点类型', () => {
    ...
    // 获取第一个节点（文本）
    const result1 = getFirstDomNode(container, 0)
    expect(result1!.kind).toBe(NodeKind.TEXT)
    
    // 获取第二个节点（元素）
    const result2 = getFirstDomNode(container, 1)
    expect(result2!.kind).toBe(NodeKind.REGULAR_ELEMENT)
    
    // 获取第三个节点（注释）
    const result3 = getFirstDomNode(container, 2)
  ...
  it('应该处理后面跟着其他节点的片段', () => {
    ...
    // 获取 Fragment
    const fragmentResult = getFirstDomNode(container, 0)
    expect(fragmentResult!.kind).toBe(NodeKind.FRAGMENT)
    expect(fragmentResult!.nextIndex).toBe(3)
    
    // 获取 Fragment 后的节点
    const afterResult = getFirstDomNode(container, fragmentResult!.nextIndex)
  ...
  it('应该返回正确的标签名', () => {
    ...
    tags.forEach((tag, index) => {
      const result = getFirstDomNode(container, index)
